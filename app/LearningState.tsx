...
export function LearningState() {
  ...
  useEffect(() => { fetch("/api/dashboard").then((response) => response.json()).then(setData).catch(() => setData({ profile: { learningGoal: "掌握 Agent Engineering", weeklyHours: 8 }, competencies: [], error: "状态加载失败。" })); }, []);
