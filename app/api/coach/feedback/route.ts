...
export async function POST(request: Request) {
  ...
  let body: { conversationId?: string; rating?: string; reason?: string };
  ...
  const allowedReasons = ["inaccurate", "misunderstood", "unactionable", "irrelevant_source"];
  if (body.rating === "unhelpful" && !allowedReasons.includes(body.reason ?? "")) return apiError("请选择未解决的原因。", 400, "INVALID_INPUT");
  try {
    const db = getDb();
    ...
    metadata.userFeedback = { rating: body.rating, reason: body.rating === "unhelpful" ? body.reason : undefined, createdAt: new Date().toISOString() };
