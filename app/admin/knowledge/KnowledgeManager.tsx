...
export function KnowledgeManager() {
  ...
  const queueCount = (workflowCounts.review ?? 0) + (workflowCounts.index ?? 0) + (workflowCounts.failed ?? 0) + uploadAttentionCount;
  ...
  const filteredDocuments = useMemo(() => { const term = search.trim().toLowerCase(); return documents.filter((document) => (activeTab !== "queue" || ["review", "index", "failed"].includes(workflowStage(document).key)) && (!term || `${document.title} ${document.summary ?? ""} ${document.content ?? ""}`.toLowerCase().includes(term)) && (statusFilter === "all" || document.status === statusFilter) && (indexFilter === "all" || document.ingestionStatus === indexFilter)); }, [activeTab, documents, search, statusFilter, indexFilter]);
  ...
  async function batchImport() {
    ...
    try {
      ...
      setNotice(`导入已受理：文件排队 ${queued}，网页新增 ${imported}，重复跳过 ${duplicates}，失败 ${failed}${failures.length ? `。${failures.slice(0, 3).join("；")}` : ""}`);
  ...
  async function bulkAction(action: "approve" | "archive" | "restore" | "index", explicitIds?: string[]) {
    ...
    try {
      ...
      setNotice(action === "index" ? `已创建 ${data.queued ?? 0} 个后台向量化任务，失败 ${data.failed ?? 0}。` : action === "archive" ? `已归档 ${data.changed ?? 0} 份资料，并立即清理召回索引。` : action === "restore" ? `已恢复 ${data.changed ?? 0} 份资料到待审批，确认内容后才能重新向量化。` : `已审批 ${data.changed ?? 0} 份资料；请确认后再单独向量化。`);
  ...
  return <main className="admin-shell knowledge-admin"><header className="admin-header"><div><Link href="/" className="back-link">← 返回学习页</Link><h1>知识库管理</h1><p>录入、审核、切片并向量化 Agent Engineering 资料。</p></div><Link href="/admin/ai" className="secondary-button">AI 渠道</Link></header>
    ...
    {activeTab === "queue" && <>
    <section className="upload-monitor channel-card"><div className="knowledge-list-heading"><div><h2>文件解析任务</h2><p>用户和管理员上传的原文件先进入 R2，再由 Queue 异步解析为待审批资料；失败原文件保留 7 天。</p></div><span>{submissions.length} 条</span></div>{submissions.length === 0 ? <p className="empty-copy">暂无文件上传任务。</p> : <div className="upload-task-list">{submissions.map((submission) => <article key={submission.id}><div className="upload-task-main"><div className="knowledge-badges"><span className={`stage-${submission.status}`}>{submission.status === "queued" ? "等待解析" : submission.status === "processing" ? "正在解析" : submission.status === "completed" ? "解析完成" : submission.status === "withdrawn" ? "用户已撤回" : "解析失败"}</span><span>第 {submission.attempt} 次处理</span><span>{(submission.fileSize / 1024 / 1024).toFixed(2)} MB</span></div><strong>{submission.fileName}</strong><p>{submission.submitterName || submission.submittedBy} · {new Date(submission.createdAt).toLocaleString("zh-CN")}</p>{submission.status === "completed" && <small>{submission.conversion || "文档转换"} · {submission.characterCount.toLocaleString()} 字符 · {submission.partCount} 份草稿 · 重复 {submission.duplicateCount}</small>}{submission.error && <em>{submission.error}</em>}</div>{submission.status === "failed" && <div className="upload-task-actions"><button className="primary-button" disabled={!!working || !submission.objectKey} onClick={() => void retryUpload(submission)}>{working === `upload-${submission.id}` ? "重试中…" : submission.objectKey ? "重新解析" : "原文件已清理"}</button>{submission.objectKey && <button className="danger-button" disabled={!!working} onClick={() => void discardUpload(submission)}>{working === `discard-${submission.id}` ? "清理中…" : "清理原文件"}</button>}</div>}</article>)}</div>}</section>
    ...
    <section className="knowledge-list">
      <div className="knowledge-list-heading"><div><h2>{activeTab === "queue" ? "待处理资料" : "全部资料"}</h2>{activeTab === "queue" && <p>这里只显示待审批、待向量化和索引失败的资料。</p>}</div><span>显示 {filteredDocuments.length} / {activeTab === "queue" ? queueCount : documents.length} 份</span></div>
