...
export function KnowledgeManager() {
  ...
  const load = useCallback(async () => { const response = await fetch("/api/admin/knowledge"); const data = await response.json(); if (!response.ok) throw new Error(data.error ?? "加载失败"); setDocuments(data.documents); }, []);
  useEffect(() => { fetch("/api/admin/knowledge").then((response) => response.json().then((data) => ({ response, data }))).then(({ response, data }) => { if (!response.ok) throw new Error(data.error ?? "加载失败"); setDocuments(data.documents); }).catch((error) => setNotice(error.message)).finally(() => setLoading(false)); }, []);
  ...
  async function indexDocument(document: DocumentItem) {
    ...
    try { const response = await fetch(`/api/admin/knowledge/${document.id}/index`, { method: "POST" }); const data = await response.json(); if (!response.ok) throw new Error(data.error ?? "索引失败"); await load(); setNotice(`索引完成：${data.chunkCount} 个切片。`); }
  ...
  return <main className="admin-shell knowledge-admin"><header className="admin-header"><div><Link href="/" className="back-link">← 返回学习页</Link><h1>知识库管理</h1><p>录入、审核、切片并向量化 Agent Engineering 资料。</p></div><Link href="/admin/ai" className="secondary-button">AI 渠道</Link></header>
    ...
    <section className="knowledge-list"><div className="knowledge-list-heading"><h2>资料列表</h2><span>{documents.length} 份</span></div>{loading ? <p className="empty-copy">正在读取…</p> : documents.length === 0 ? <p className="empty-copy">还没有资料，请先录入第一份。</p> : documents.map((document) => <article className="knowledge-item" key={document.id}><div className="knowledge-item-main"><div className="knowledge-badges"><span>{document.status === "approved" ? "已审核" : document.status === "archived" ? "已归档" : "草稿"}</span><span className={document.ingestionStatus === "failed" ? "failed" : ""}>{document.ingestionStatus}</span><span>{document.chunkCount} chunks</span></div><h3>{document.title}</h3><p>{document.summary || document.content?.slice(0, 150)}</p><small>{document.versionLabel || "无版本"} · {document.trustLevel} · 更新于 {new Date(document.updatedAt).toLocaleString("zh-CN")}</small>{document.ingestionError && <em>{document.ingestionError}</em>}</div><div className="knowledge-actions"><button className="secondary-button" onClick={() => edit(document)}>编辑</button><button className="primary-button" disabled={document.status !== "approved" || working === document.id} onClick={() => indexDocument(document)}>{working === document.id ? "处理中…" : document.ingestionStatus === "indexed" ? "重建索引" : "建立索引"}</button><button className="danger-button" disabled={working === document.id} onClick={() => remove(document)}>删除</button></div></article>)}</section>
  </main>;
}
