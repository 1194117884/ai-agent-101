...
export function KnowledgeManager() {
  ...
  const queueCount = (workflowCounts.review ?? 0) + (workflowCounts.index ?? 0) + (workflowCounts.failed ?? 0);
  ...
  const load = useCallback(async () => { const response = await fetch("/api/admin/knowledge"); const data = await response.json(); if (!response.ok) throw new Error(data.error ?? "加载失败"); setDocuments(data.documents); setStats(data.stats); setRetrievalLogs(data.retrievalLogs ?? []); setJobs(data.jobs ?? []); }, []);
  ...
  useEffect(() => { Promise.all([fetch("/api/admin/knowledge").then((response) => response.json().then((data) => ({ response, data }))), fetch("/api/admin/knowledge/evals").then((response) => response.json().then((data) => ({ response, data })))]).then(([knowledge, evals]) => { if (!knowledge.response.ok) throw new Error(knowledge.data.error ?? "加载失败"); setDocuments(knowledge.data.documents); setStats(knowledge.data.stats); setRetrievalLogs(knowledge.data.retrievalLogs ?? []); setJobs(knowledge.data.jobs ?? []); if (evals.response.ok) setEvalCases(evals.data.cases ?? []); }).catch((error) => setNotice(error.message)).finally(() => setLoading(false)); }, []);
