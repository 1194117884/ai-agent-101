...
export function KnowledgeManager() {
  ...
  const load = useCallback(async () => { const response = await fetch("/api/admin/knowledge"); const data = await response.json(); if (!response.ok) throw new Error(data.error ?? "加载失败"); setDocuments(data.documents); setStats(data.stats); }, []);
  useEffect(() => { fetch("/api/admin/knowledge").then((response) => response.json().then((data) => ({ response, data }))).then(({ response, data }) => { if (!response.ok) throw new Error(data.error ?? "加载失败"); setDocuments(data.documents); setStats(data.stats); }).catch((error) => setNotice(error.message)).finally(() => setLoading(false)); }, []);
