...
export function KnowledgeManager() {
  ...
  return <main className="admin-shell knowledge-admin"><header className="admin-header"><div><Link href="/" className="back-link">← 返回学习页</Link><h1>知识库管理</h1><p>录入、审核、切片并向量化 Agent Engineering 资料。</p></div><Link href="/admin/ai" className="secondary-button">AI 渠道</Link></header>
    ...
    {activeTab === "queue" && <>
    <section className="upload-monitor channel-card"><div className="knowledge-list-heading"><div><h2>文件解析任务</h2><p>用户和管理员上传的原文件先进入 R2，再由 Queue 异步解析为待审批资料。</p></div><span>{submissions.length} 条</span></div>{submissions.length === 0 ? <p className="empty-copy">暂无文件上传任务。</p> : <div className="upload-task-list">{submissions.map((submission) => <article key={submission.id}><div className="upload-task-main"><div className="knowledge-badges"><span className={`stage-${submission.status}`}>{submission.status === "queued" ? "等待解析" : submission.status === "processing" ? "正在解析" : submission.status === "completed" ? "解析完成" : "解析失败"}</span><span>第 {submission.attempt} 次处理</span><span>{(submission.fileSize / 1024 / 1024).toFixed(2)} MB</span></div><strong>{submission.fileName}</strong><p>{submission.submitterName || submission.submittedBy} · {new Date(submission.createdAt).toLocaleString("zh-CN")}</p>{submission.status === "completed" && <small>{submission.conversion || "文档转换"} · {submission.characterCount.toLocaleString()} 字符 · {submission.partCount} 份草稿 · 重复 {submission.duplicateCount}</small>}{submission.error && <em>{submission.error}</em>}</div>{submission.status === "failed" && <button className="primary-button" disabled={!!working || !submission.objectKey} onClick={() => void retryUpload(submission)}>{working === `upload-${submission.id}` ? "重试中…" : submission.objectKey ? "重新解析" : "原文件已清理"}</button>}</article>)}</div>}</section>
