...
export function KnowledgeManager() {
  ...
  async function bulkAction(action: "approve" | "archive" | "restore" | "index", explicitIds?: string[]) {
    ...
    try {
      ...
      setNotice(action === "index" ? `批量向量化完成：成功 ${data.completed ?? 0}，失败 ${data.failed ?? 0}。` : action === "archive" ? `已归档 ${data.changed ?? 0} 份资料，并立即清理召回索引。` : action === "restore" ? `已恢复 ${data.changed ?? 0} 份资料到待审批，确认内容后才能重新向量化。` : `已审批 ${data.changed ?? 0} 份资料；请确认后再单独向量化。`);
