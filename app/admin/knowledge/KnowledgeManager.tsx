...
export function KnowledgeManager() {
  ...
  async function batchImport() {
    ...
    try {
      ...
      setNotice(`导入完成：新增 ${data.imported ?? 0}，重复跳过 ${data.duplicates ?? 0}，失败 ${data.failed ?? 0}${failures.length ? `。${failures.join("；")}` : ""}`);
